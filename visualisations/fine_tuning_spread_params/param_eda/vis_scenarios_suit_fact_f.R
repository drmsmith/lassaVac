...
main_dir <- "res/scenarios_suit_fact_f_09875" # "res/scenarios_x5" trial scenarios 
scenario_id <- 'suit_fact_f_'
suit_factors <- c(1, 0.9, 0.7, 0.5)
pltname = 'suitability_fact_f_scenarios_spread.png'
plt_ncols = 4 ### CHANGE TO 5
### str_replace('suit_fact_k', 'factor ')
### width=3300, height=1400, 


resdirs <- dir(main_dir, pattern = scenario_id, full.names=T) %>%
    str_sort(numeric=T)
...
filepaths <- filepaths[!str_detect(filepaths, 'exclude')]

ids = dir(main_dir, pattern = scenario_id) %>%
    str_sort(numeric=T, decreasing = T) %>% str_replace('suit_fact_f_', 'factor ')
    ##### DECREASING FOR SUITABILITY 
...
df_tranformed_incidences = map(suit_factors, function(.inc_fact) {
    df_out = data.frame(
        annual_incidence = .inc_fact * df_burden$mean_pd_weighted, 
        code = df_burden$code, 
...
ggsave(trans_suit_hists,
    filename=file.path('figs','trans_suit_hists.png'), dpi=330, 
...
plts
ggsave(plts,
    filename=file.path('figs','trans_suit_fact_f_hists_spread.png'), dpi=330, 
...
plts
ggsave(plts,
    filename=file.path('figs','hists_100_160_suit_fact_f.png'), dpi=330, 
    width=3500, height=2400, units='px')
