# LASV infection pruning method.
# Uses reformatted output from running branching process model.
...
library(tidyverse)
source("Lassa_pruning_functions_v15.R")
...
for (i in (0:98)) {
  ...
  model_outputs <- read_csv(paste0("OxLiv_lassa_Routput_R0_0631_v6C_reformatted_", i, ".csv"))
  ...
  write.csv(Lassa_results_all, file=sprintf("Lassa_results_v15_%s_v6C99_%s_scenarios_all.csv", VE, (i+1)), row.names=FALSE)
