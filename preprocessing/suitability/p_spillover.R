...
df_burden <- read.csv("data/sum_chik_incidence_by_country_full.csv") %>%
    dplyr::select(country, incidence) %>%
    rename(
        code = country,
...
df_burden$country <- countrycode::countrycode(df_burden$code, "iso3c", "country.name")
# Some values were not matched unambiguously: PIS, SP-, XAD, XCA, XCL, XKO, XNC
...
df_burden_p_spillover <- right_join(df_burden, df_suit_means_who, by = join_by(code == country_code)) %>%
    ...
    drop_na() %>% # rename for consistency downstream
    rename(mean_ppp_weighted = mean_pop_weighted)



# check and save
dim(df_burden_p_spillover)
head(df_burden_p_spillover)
