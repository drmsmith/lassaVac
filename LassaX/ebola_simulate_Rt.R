...
# outbreak dates
df_ebola_garske_cases_by_t_district_drange = data.frame(
  dates = seq(min(df_ebola_garske_cases_by_t_district_incompl$dates), 
...
# Plot cases
p_ebola_incidence_t_district = ggplot(df_ebola_garske_cases_by_t_district, 
                                      aes(x = dates, y = I, colour = district)) +
...
df_catchments_ebola_SLE = read.csv("LassaX/data/districts_sierraleone.csv", 
                                   stringsAsFactors = F) %>%
...
df_catchments_ebola_GIN = read.csv("LassaX/data/districts_guinea.csv", 
                                   stringsAsFactors = F) %>%
  mutate(Population = gsub(",", "", Population),
...
df_catchments_ebola_LBR = read.csv("LassaX/data/incidence_ADM_1_PB_2_JT1.csv",
                                   stringsAsFactors = F) %>%
...
model_cases_population = glm(Cases_cumulative_log10 ~ Population_log10, 
                             family = gaussian, 
                             data = df_catchments_ebola_cases %>%
...
for(outbreak_i in 1:length(list_Rt_ebola_i)) {
  ...
  if (length(vec_estimates_shape_i) != length(vec_estimates_scale_i)) {
    warning("unequal number of posterior estimates")
  }
  ...
  colnames(m_Rt_simulated) <-
    c(
      "district",
      "district_outbreak",
      "outbreak_all",
      "quantile",
      1:(n_Rt_estimates + length_dissipateRt)
    )
  ...
  for (quantile_j in vec_quantiles_j) {
    ...
    vec_Rt_sims_extension = sapply(1:length_dissipateRt, function(x)
      tail(Rt_sims_j, 1) * (1 - (x / length_dissipateRt)))
    Rt_sims_j = append(Rt_sims_j, vec_Rt_sims_extension)
    
    m_Rt_simulated[qounter_j,] <-
      c(district_i,
        district_outbreak_i,
        outbreak_all_i,
        quantile_j,
...
df_Rt_ebola_i = as.data.frame(list_Rt_ebola_i[[3]][["m_Rt"]]) %>%
  ...
  pivot_longer(-c(district, district_outbreak, outbreak_all, quantile, quantile_factor),
               names_to = "day", values_to = "Rt") %>%
...
df_Rt_ebola_quantiles_1 = as.data.frame(list_Rt_ebola_i[[1]][["m_Rt"]]) %>%
  pivot_longer(-c(district, district_outbreak, outbreak_all, quantile), 
               names_to = "day", values_to = "Rt") %>%
  mutate(day = as.numeric(day),
         Rt = as.numeric(Rt)) %>%
...
# save(list_Rt_ebola_i, file = "LassaX/data/inputs_list_Rt_curves.Rdata")
